# kennisplatformvhg.nl
*Last update:2024-01-11 16:13:41*
## Summary data
| File       | Count | Info |
|------------|-------|------|
|[subdomains.txt](/data/kennisplatformvhg/subdomains.txt)|2|Alive subdomains|
|[urls.txt](/data/kennisplatformvhg/urls.txt)|2|Urls|
|[all_subdomains.txt](/data/kennisplatformvhg/all_subdomains.txt)|2|All found subdomains (history)|
## Statistics
| Url | SSL | Server | Cookie | HSTS | CSP | XFO | XXP | RP | Tech |
|------------|-------|------|------|------|------|------|------|------|------|
|[http://www.kennisplatformvhg.nl](http://www.kennisplatformvhg.nl)| | | | | | | |:white_check_mark: ||
|[https://kennisplatformvhg.nl](https://kennisplatformvhg.nl)| |apache| |:white_check_mark: | | | |:white_check_mark: |Apache HTTP Server H...|